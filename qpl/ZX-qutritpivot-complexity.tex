...
\def\titlerunning{Tensor-Network Rewriting as a\\
Unifying Diagnostic of Complexity}
...
Here we give a quick introduction to the qubit ZX-calculus. For a more detailed treatment, see Chapter 9.4 in \cite{PQP}. The qubit ZX-calculus is a diagrammatic language for quantum mechanics consisting of \textit{spiders} connected together by \textit{wires}, living within a rectangular plane. Diagrams are read bottom-to-top. Wires that don't end at a spider are \textit{inputs} (if their non-spider end starts at the bottom of the plane) and/or \textit{outputs} (if their non-spider end lies at the top of the plane).\newline
