...
% \label{eq:link}
% \end{equation}
We say that $L\simeq L'$ iff the diagram of link $L$ can be deformed to that of link $L'$ without cutting, gluing or intersecting the strands.
...
of topological quantum computation \cite{pachos_2012}, which is most natural for knot theory, 
...
For a universal anyonic theory, any unitary evolution can be efficiently approximated by a braid.
...
Remarkably, $V_L(t(d))$ can be expressed as an unphysical partition function $Z_{G_L}(d)$ of a $d$-state Potts model with suitable spin-spin interactions \cite{RevModPhys.64.1099}.
...
\begin{equation}\label{eq:link-tn}
...
except that for $k=0,1,2$ the problem is in P \cite{jaeger_vertigan_welsh_1990}.
For $d=2$, the $X$-matrix is just the Pauli X, and so the problem reduces to simplifying a stabilizer diagram,
...
However, for $d=3$ the $X$-matrix cannot be expressed as a stabilizer qutrit diagram.
	...
	\end{equation}
So it is not expected that there exists an efficient simplification strategy for the corresponding qutrit ZX diagram. This is consistent with the fact that counting 3-colourings is \#P-complete.
