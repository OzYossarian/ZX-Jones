...
% For the lattice roots of unity, the quantum amplitude to which the problem reduces can be computed efficiently classically.
% Specifically, the quantum circuits that need to be simulated are stabilizer circuits which are known to be classically tractable [ref? Kuperberg? Aharonov?].
...
The partition function $Z_{G_L}(d\in\mathbb{N})$ can be expressed as a closed tensor network in terms of phaseless (green) $d$-dimensional $Z$-spiders connected via wires that go through $\pm$-boxes \cite{PhysRevE.100.033303}:
\begin{equation}
\includegraphics[scale=0.4]{figures/link-tn.pdf}
\label{eq:link-tn}
\end{equation}
The $\pm$-boxes have the following concrete interpretation as matrices:
	\begin{equation}\label{eq:pm_tensor}
\left\llbracket \ \tikzfig{pm_maps/pm} \ \right\rrbracket \quad = \quad  \sum_{i,j=0}^{d-1}(1 - (1+t(d)^{\mp 1}) \delta_{ij} ) \ket{i}\bra{j} ~~.
	\end{equation}
...
In keeping with the spirit of our graphical exposition, we recall that computing partition functions and counting problems are essentially the same problem, as they can be straightforwardly encoded as closed tensor networks \cite{10.21468/SciPostPhys.7.5.060,debeaudrap2020tensor}.

Given a graph $G$, the graph colouring problem can be encoded as a ZX diagram as follows. Every vertex of the graph is mapped to a $d$-dimensional (green) phaseless $Z$-spider.
Every edge is mapped to a wire connecting the spiders
which goes through an $X$-box.
\begin{equation}
\includegraphics[scale=0.4]{figures/G-tn.pdf}
\label{eq:link-tn}
\end{equation}
...
\left\llbracket \ \tikzfig{pm_maps/x} \ \right\rrbracket \quad = \quad  \sum_{i,j=0}^{d-1} (1 -  \delta_{ij} ) \ket{i}\bra{j} ~~.
	\end{equation}
Indeed, they are just a special case of the $\pm$-boxes in \eqref{eq:pm_tensor} for $t=0$.

It is known that for arbitrary graphs 2-colouring is in P {\bf [ref]}.
For $d=2$, we trivially notice that the $X$-matrix is just the Pauli X, as shown in \eqref{eq:XmatricesinZX}, and so the problem reduces to simplifying a stabiliser diagram,
which can be done efficiently.
However, for $d=3$ the $X$-matrix cannot be expressed as a stabilizer qutrit diagram, and so the problem is not expected that there exists an efficient simplification strategy for the corresponding qutrit ZX diagram. This is consistent with the fact that 3-colouring is \#P-complete {\bf [ref]}.
