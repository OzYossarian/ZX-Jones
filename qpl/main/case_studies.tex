...
However, for $d=3$ the $X$-matrix cannot be expressed as a stabilizer qutrit diagram, and so it is not expected that there exists an efficient simplification strategy for the corresponding qutrit ZX diagram. This is consistent with the fact that 3-colouring is \#P-complete {\bf [ref]}.
	\begin{equation}\label{eq:XmatricesinZX}
		\left\llbracket \ \tikzfig{pm_maps/x} \ \right\rrbracket_{d=2} \simeq 
		\left\llbracket \ \tikzfig{pm_maps/q2x} \ \right\rrbracket \quad, 
		\qquad\quad
		\left\llbracket \ \tikzfig{pm_maps/x} \ \right\rrbracket_{d=3} \not\simeq
		\left\llbracket \ \tikzfig{pm_maps/q3x} \ \right\rrbracket~~,~~~~\forall a,b\in\{0,1,2\}~.
	\end{equation}
